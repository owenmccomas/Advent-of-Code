const sort = (a: Array<any>) => {
  const lines = a;
  let sum = 0;

  for (const line of lines) {
    const { firstDigit, lastDigit } = findFirstAndLast(line);

    if (firstDigit !== null && lastDigit !== null) {
      const twoDigitValue = parseInt(`${firstDigit}${lastDigit}`, 10);
      sum += twoDigitValue;
    }
  }
  ...
  console.log(`Total Sum: ${greenColor}${sum}${resetColor}`);
};

const findFirstAndLast = (
  line: string
): { firstDigit: number | null; lastDigit: number | null } => {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    if (isDigit(currentChar)) {
      if (firstDigit === null) {
        firstDigit = parseInt(currentChar, 10);
      }
      lastDigit = parseInt(currentChar, 10);
    } else {
      const spelledOutNumber = findSpelledOutNumber(line.substr(i));
      if (spelledOutNumber !== null) {
        if (firstDigit === null) {
          firstDigit = spelledOutNumber;
        }
        lastDigit = spelledOutNumber;
      }
    }
  }

  console.log(
    `Line: ${line}, First Digit: ${firstDigit}, Last Digit: ${lastDigit}`
  );
...
const findSpelledOutNumber = (substring: string): number | null => {
  ...
  for (const digit in spelledDigits) {
    if (substring.indexOf(digit) === 0) {
